  ...
  braintree.setup(data.client_token, "custom", {
    ...
    onPaymentMethodReceived: function(obj) {
      ...
      if(obj.type == 'PayPalAccount') {
        data['email'] = obj.details.email;
        $("#cc_type_br").val('paypal');
        $("#last_digits_br").remove();
      } else {
        $("#last_digits_br").val(obj.details.lastTwo);
        $("#cc_type_br").val(CARD_TYPE_MAPPING[obj.details.cardType]);
      }

      $('#data_br').val(JSON.stringify(data));
